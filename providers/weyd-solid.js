...
(function (global) {
  ...
  var MAX_HTML_ROWS = 80;
  var MAX_STREAMS_PER_PAGE = 15;
  ...
  function metaMatchesTitle(meta, scrapedTitle) {
    ...
    var tNorm = normalizeForMatch(scrapedTitle);
    var showNorm = normalizeForMatch(searchTitle);
  ...
  function extractHtmlRows(html, containerPath) {
    ...
    if (containerPath.indexOf('tr') >= 0) {
      var trRe = /<tr[\s>][\s\S]*?<\/tr>/gi;
      var tm;
      while ((tm = trRe.exec(html)) !== null) rows.push(tm[0]);
      return rows.length ? rows : [html];
    }
    return [html];
  }
  ...
  function filterDrillLinks(html, source, pageUrl, meta) {
    ...
    for (var i = 0; i < links.length; i++) {
      ...
      if (
        ...
        if (!metaMatchesTitle(meta, text)) continue;
      }
  ...
  function getStreams(tmdbId, mediaType, season, episode) {
    ...
    return getMeta(tmdbId, mediaType, season, episode)
      ...
      .then(function (streams) {
        var out = dedupeStreams(streams || []);
        console.log(
          '[' + LOG_TAG + '] matched ' + (streams ? streams.length : 0) +
            ', returning ' + out.length
        );
        return out;
      })
      .catch(function (err) {
