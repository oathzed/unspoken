...
(function (global) {
  ...
  var GROUP_CONCURRENCY = 3;
  var DRILL_CONCURRENCY = 2;
  var MAX_DRILL_LINKS = 6;
  var MAX_TOTAL_STREAMS = 40;
  var MIN_STREAMS_NEXT_GROUP = 5;
  var TARGET_STREAMS_STOP = 25;
  ...
  function getMeta(tmdbId, mediaType, season, episode) {
    ...
    return fetchJson(url)
      ...
      .catch(function (err) {
        console.log('[WeydTorrent] TMDB failed: ' + (err && err.message));
        return {
          title: 'Unknown',
          year: null,
          imdb_id: null,
  ...
  function resolveUrl(base, href) {
    ...
    var dir = baseNoQuery.lastIndexOf('/') >= 0
  ...
  function runGroup(urls, meta, existingCount) {
    var collected = [];
    var i = 0;
    function nextSource() {
      if (i >= urls.length || collected.length >= TARGET_STREAMS_STOP) {
      ...
      return loadSource(sourceUrl)
        ...
        .then(function (streams) {
          if (streams && streams.length) {
            collected = collected.concat(streams);
          }
          return nextSource();
        })
        .catch(function (err) {
          console.log(
            '[WeydTorrent] source fail: ' +
              sourceUrl +
              ' ' +
              (err && err.message)
          );
  ...
  function runAllGroups(primary, meta) {
    ...
    function nextGroup() {
      ...
      return runGroup(urls, meta, all.length).then(function (found) {
        ...
        if (dedupeStreams(all).length >= MIN_STREAMS_NEXT_GROUP) {
  ...
  function getStreams(tmdbId, mediaType, season, episode) {
    ...
    return getMeta(tmdbId, mediaType, season, episode)
      .then(function (meta) {
        if (!meta.imdb_id) {
          console.log(
            '[WeydTorrent] Warning: no IMDB id — API sources may be skipped'
          );
        }
        return loadPrimary()
          ...
          .catch(function (err) {
            console.log('[WeydTorrent] Primary/scrape error: ' + (err && err.message));
            return [];
          });
      })
      .catch(function (err) {
        console.log('[WeydTorrent] Fatal: ' + (err && err.message));
        return [];
      });
  }
