/**
 * Nuvio provider — runs Weyd Primary + Source JSON scrapers (Weyd bridge).
 * Primary manifest: lalitjoshi06/Scrapers sample/scrapers.json
 */
(function (global) {
  'use strict';
  ...
  var GROUP_CONCURRENCY = 5;
  var DRILL_CONCURRENCY = 3;
  var MAX_DRILL_LINKS = 8;
  var MAX_PAGES = 5;
  var MAX_TOTAL_STREAMS = 45;
  var MIN_STREAMS_NEXT_GROUP = 8;
  ...
  /** Java String.format-style replacement (Weyd search URLs). */
  ...
  function buildMagnet(hashOrMagnet, displayName) {
    ...
    var hash = s.replace(/[^a-fA-F0-9]/g, '');
    if (hash.length === 32) hash = hash.toLowerCase();
  ...
  function streamFromDirect(opts) {
    ...
    return {
      name: opts.sourceName,
      title: (opts.title || opts.url).substring(0, 120),
      url: opts.url,
  ...
  function getTmdbApiKey() {
    try {
      if (typeof global !== 'undefined' && global.SCRAPER_SETTINGS && global.SCRAPER_SETTINGS.tmdbApiKey) {
        return String(global.SCRAPER_SETTINGS.tmdbApiKey);
      }
    } catch (e) {}
    return TMDB_API_KEY;
  }
  ...
  // ---------------------------------------------------------------------------
  // Primary + Source JSON loading
  // ---------------------------------------------------------------------------

  function loadPrimary() {
    if (primaryCache) return Promise.resolve(primaryCache);
    return fetchJson(PRIMARY_URL).then(function (data) {
      primaryCache = data;
      return data;
    });
  }
  ...
  function loadSource(url) {
    if (sourceJsonCache[url]) return Promise.resolve(sourceJsonCache[url]);
    return fetchJson(url).then(function (data) {
      sourceJsonCache[url] = data;
      return data;
    });
  }

  // ---------------------------------------------------------------------------
  // API scraper (Weyd API sources)
  // ---------------------------------------------------------------------------
    ...
    for (var i = 0; i < pairs.length; i++) {
      ...
      if (api.api_title_split && title) {
        var parts = String(title).split(
          new RegExp(api.api_title_split)
        );
      ...
      var quality =
        apiField(row, parent, api, 'path_quality') ||
  ...
  // ---------------------------------------------------------------------------
  // HTML helpers (subset of Weyd / Jsoup behavior)
  // ---------------------------------------------------------------------------
  ...
  function hrefMatchesRule(href, rule) {
    ...
    var contains = rule.match(/:contains\(([^)]+)\)/);
  ...
  function filterDrillLinks(html, source, pageUrl) {
    ...
    var attr = source.drill_down_path_attr || 'href';
    var useAbs = attr.indexOf('abs:') === 0;
    ...
    for (var i = 0; i < links.length; i++) {
      ...
      var full = useAbs ? resolveUrl(pageUrl, href) : resolveUrl(pageUrl, href);
      if (!seen[full]) {
        seen[full] = true;
        out.push(full);
      }
  ...
  function extractAttrFromHtml(html, selector, attr) {
    if (!selector || selector.indexOf('|') === 0) return '';
    var magnets = extractAllMagnets(html);
    if (
      selector.indexOf('magnet') >= 0 ||
      (attr === 'href' && magnets.length)
    ) {
      return magnets[0] || '';
    }
    if (selector.indexOf('h1') >= 0) {
  ...
  function scrapeHtml(source, meta) {
    ...
    return fetchText(url).then(function (html) {
      ...
      var streams = scrapeHtmlPage(html, source, url);
      if (streams.length) return streams;
      if (source.more_pages_indicator && html.indexOf(source.more_pages_indicator) >= 0) {
        return streams;
      }
      return streams;
    });
  ...
  function runGroup(urls, meta) {
    return mapLimit(urls, GROUP_CONCURRENCY, function (sourceUrl) {
      return loadSource(sourceUrl)
        .then(function (src) {
          return scrapeSource(src, meta);
        })
        .catch(function (err) {
          console.log('[WeydTorrent] source fail: ' + sourceUrl + ' ' + (err && err.message));
          return [];
        });
    }).then(flatten);
  }
  ...
  function runAllGroups(primary, meta) {
    var groups = ['1', '2', '3', '4', '5'];
    var all = [];
    var gi = 0;

    function nextGroup() {
      if (gi >= groups.length || all.length >= MAX_TOTAL_STREAMS) {
        return Promise.resolve(dedupeStreams(all));
      }
      var key = groups[gi++];
      var urls = primary[key];
      if (!urls || !urls.length) return nextGroup();
      console.log('[WeydTorrent] Group ' + key + ' (' + urls.length + ' sources)');
      return runGroup(urls, meta).then(function (found) {
        all = all.concat(found);
        console.log('[WeydTorrent] Group ' + key + ' returned ' + found.length + ' (total ' + all.length + ')');
        if (all.length >= MIN_STREAMS_NEXT_GROUP) {
  ...
  function getStreams(tmdbId, mediaType, season, episode) {
    ...
    return getMeta(tmdbId, mediaType, season, episode)
      .then(function (meta) {
        if (!meta.imdb_id) {
          console.log('[WeydTorrent] Warning: no IMDB id — API sources may be skipped');
        }
        return loadPrimary().then(function (primary) {
          return runAllGroups(primary, meta);
        });
      })
      .catch(function (err) {
        console.log('[WeydTorrent] Error: ' + (err && err.message));
        return [];
      });
  }
